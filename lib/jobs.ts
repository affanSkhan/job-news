...
function dateString(v:any){if(v instanceof Date&&Number.isFinite(v.getTime()))return v.toISOString();if(typeof v==="number"){const d=new Date(v);return Number.isFinite(d.getTime())?d.toISOString():""}if(typeof v==="string"&&v.trim()){const d=new Date(v);return Number.isFinite(d.getTime())?d.toISOString():v}return ""}
function decodeEntities(s:string){let out=String(s||"");for(let i=0;i<3;i++){const next=out.replace(/&nbsp;/gi," ").replace(/&amp;/gi,"&").replace(/&quot;/gi,'"').replace(/&#39;|&#x27;/gi,"'").replace(/&lt;/gi,"<").replace(/&gt;/gi,">").replace(/&#x2F;|&#47;/gi,"/").replace(/&#(\d+);/g,(_,n)=>String.fromCharCode(Number(n))).replace(/&#x([0-9a-f]+);/gi,(_,n)=>String.fromCharCode(parseInt(n,16)));if(next===out)break;out=next}return out}
export function cleanJobText(value:string){return decodeEntities(String(value||"")).replace(/<br\s*\/?\s*>/gi,"\n").replace(/<\/p>/gi,"\n\n").replace(/<\/li>/gi,"\n").replace(/<[^>]*>/g," ").replace(/[ \t]+\n/g,"\n").replace(/\n[ \t]+/g,"\n").replace(/[ \t]{2,}/g," ").replace(/\n{3,}/g,"\n\n").trim()}
export function canonicalApplicationUrl(raw:string){try{const u=new URL(String(raw||""));u.hash="";const host=u.hostname.toLowerCase().replace(/^www\./,"");if(host==="boards.greenhouse.io"||host==="job-boards.greenhouse.io"||host==="boards-api.greenhouse.io")u.hostname="boards.greenhouse.io";for(const key of [...u.searchParams.keys()]){if(/^utm_/i.test(key)||/^(source|ref|referrer|gh_jid)$/i.test(key))u.searchParams.delete(key)}u.pathname=u.pathname.replace(/\/+$/,"");u.search=u.searchParams.toString()?"?"+u.searchParams.toString():"";return u.toString()}catch{return String(raw||"").trim()}}
function jobCompanyKnown(name:string){return Boolean(String(name||"").trim())&&!/^(unknown company|unknown|n\/a|not disclosed)$/i.test(String(name||"").trim())}
function jobQuality(j:Job){return(jobCompanyKnown(j.company)?100:0)+Math.min(35,j.description.length/500)+(j.category&&j.category!=="Other"?10:0)+(j.verified?5:0)}
