  ...
  return (
    <main>
      ...
      <div className="container section">
        ...
        <div style={{margin:"24px 0"}}><ResumeUpload/></div>
        ...
        <div className="grid" style={{marginTop:18}}>
          <section className="card">
            <div className="eyebrow">Profile controls</div>
            <h2>Tell Radar what matters</h2>
            <ProfileForm profile={profile}/>
          </section>

          <section className="card">
            <div className="eyebrow">Notifications</div>
            <h2>Stay ahead of fresh roles</h2>
            <AlertForm/>
            <h3>Active alerts</h3>
            <p>{alerts.length>0 ? alerts.map((a:any)=>a.name+" · "+a.frequency).join(" | ") : "No alerts yet."}</p>
          </section>
        </div>
        ...
        <div className="grid" style={{marginTop:18}}>
          <section className="card">
            ...
            <h2>Saved jobs</h2>
            {saved.length>0 ? (
              <ul>{saved.map((x:any)=><li key={x.job_id}><Link href={"/jobs/"+x.slug}>{x.title}</Link> — {x.company_name}</li>)}</ul>
            ) : (
          ...
          <section className="card">
            ...
            {apps.length>0 ? (
              <ul>{apps.map((x:any)=><li key={x.job_id}><Link href={"/jobs/"+x.slug}>{x.title}</Link> — {x.company_name} · <StatusSelect jobId={x.job_id} status={x.status}/></li>)}</ul>
            ) : (
