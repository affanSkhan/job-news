...
export async function generateStaticParams(){
  ...
  for(const j of getActiveJobs()){const s=String(j.category||"Other").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"");counts.set(s,(counts.get(s)||0)+1);}
  return [...counts.entries()].filter(([,n])=>n>=3).map(([slug])=>({slug}));
}
export default async function CategoryPage({params}:{params:Promise<{slug:string}>}){
 const {slug}=await params;const jobs=getActiveJobs().filter(j=>String(j.category||"Other").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")===slug);
