...
export default async function India(){
 ...
 return <main>
  <header className="nav"><div className="container navin"><Link href="/" className="brand">Job<span>News</span></Link><nav className="links"><Link href="/jobs">Explore</Link><Link className="active-nav" href="/india">🇮🇳 India</Link><Link href="/internships">Internships</Link><Link href="/remote-jobs">Remote</Link><Link href="/companies">Companies</Link><Link href="/skills">Skills</Link><Link href="/locations">Locations</Link><Link href="/account">Your Radar</Link></nav></div></header>
  <section className="india-hero"><div className="container"><div className="eyebrow">India opportunity radar</div><h1>Jobs for India, without the noise.</h1><p>Direct employer and ATS opportunities across Pune, Bengaluru, Hyderabad, Delhi-NCR, Mumbai and more — refreshed automatically throughout the day.</p><div className="india-stats"><div><b>{jobs.length}</b><span>India opportunities</span></div><div><b>{fresh}</b><span>found today</span></div><div><b>{IndiaRemote}</b><span>remote from India</span></div></div></div></section>
