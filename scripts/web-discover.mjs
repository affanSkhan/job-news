...
function inferExperience(title,summary){const s=String(title||"")+" "+String(summary||"");if(/not (for|open to).{0,30}(fresh|graduate|entry)/i.test(s))return"Not specified";if(/intern|internship|graduate trainee|trainee|apprentice|new grad|new graduate|fresher|entry[- ]level|campus|early career|0\s*(?:-|–|to)\s*2|0\s*[-–]\s*1\s*year|0\s*years?/i.test(s))return"fresher";if(/1\s*(?:-|–|to)\s*2\s*years?|1\s*[-–]\s*3\s*years?/i.test(s))return"0-3";if(/2\s*(?:-|–|to)\s*4\s*years?|3\s*(?:-|–|to)\s*5\s*years?/i.test(s))return"3-5";return"Not specified"}
const found=[];
